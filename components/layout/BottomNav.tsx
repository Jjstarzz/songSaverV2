...
import { Home, Music2, CalendarDays, Settings2, BookOpen } from 'lucide-react'
      ...
      <div className="flex items-stretch max-w-lg mx-auto">
        {navItems.map(({ href, label, icon: Icon, exact }) => {
          ...
          return (
            ...
            >
              {active && (
                <span className="absolute inset-x-2 top-1.5 bottom-1.5 rounded-2xl bg-accent-500/15 border border-accent-500/20" />
              )}
              <Icon className={cn('relative z-10', active ? 'w-5 h-5' : 'w-5 h-5')} />
              <span className="relative z-10 text-[10px] font-semibold leading-none tracking-wide">{label}</span>
            </Link>
          )
        })}
      </div>
