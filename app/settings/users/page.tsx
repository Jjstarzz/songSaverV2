...
export default function UsersPage() {
  ...
  const accounts = users.filter(u => !u.is_anonymous)
    ...
    <>
      ...
      <div className="px-4 pt-6 pb-10 space-y-5">
        ...
        <div className="space-y-2">
          <p className="section-label">Accounts ({accounts.length})</p>
          {accounts.length === 0
            ? <p className="text-xs text-white/30 px-1">No registered accounts yet.</p>
            : accounts.map(renderUserRow)}
        </div>

        {/* Anonymous sessions */}
        <div className="space-y-2">
          <p className="section-label">Anonymous Sessions ({anonymous.length})</p>
          {anonymous.length === 0
            ? <p className="text-xs text-white/30 px-1">No anonymous sessions.</p>
            : anonymous.map(renderUserRow)}
        </div>
